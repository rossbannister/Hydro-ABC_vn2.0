...
USE DefConsTypes, ONLY :   &
  ...
  ExtraDiagFields,         &
  A, B, C, f, g, output_ABC_file, datadirABC_out
...
INTEGER  ::  fft_dumpspacing    ! for fft levels and profiles output, Hydro
...
!!!! Initiate p0 and rho0 (for moist paramterization only) by Hydro
...
!! output levels and profiles at every step, Hydro
fft_dumpspacing  = 1/dt

!OPEN (101, file=trim(datadirABC_out)//'/'//trim(output_ABC_file)//'.lv10.bin',   &
!form="unformatted", access="direct", recl=nlongs*ZREAL8, status="replace")
!OPEN (102, file=trim(datadirABC_out)//'/'//trim(output_ABC_file)//'.lv30.bin',   &
!form="unformatted", access="direct", recl=nlongs*ZREAL8, status="replace")
!OPEN (103, file=trim(datadirABC_out)//'/'//trim(output_ABC_file)//'.lv50.bin',   &
!form="unformatted", access="direct", recl=nlongs*ZREAL8, status="replace")

!!OPEN (201, file=trim(datadirABC_out)//'/'//trim(output_ABC_file)//'.profile1.bin', &
!!form="unformatted", access="direct", recl=nlevs*ZREAL8,  status="replace")
!!OPEN (202, file=trim(datadirABC_out)//'/'//trim(output_ABC_file)//'.profile2.bin', &
!!form="unformatted", access="direct", recl=nlevs*ZREAL8,  status="replace")
!!OPEN (203, file=trim(datadirABC_out)//'/'//trim(output_ABC_file)//'.profile3.bin', &
!!form="unformatted", access="direct", recl=nlevs*ZREAL8,  status="replace")
!! output levels and profiles at every step, Hydro
...
CALL Write_state_2d (fileout, StateA, dims, ndumps+1, 0, dumpspacing, .NOT.ExtraDiagFields)

!!write(101, rec=1) StateA % w(1:nlongs, 10 )    !! Hydro
!!write(102, rec=1) StateA % w(1:nlongs, 30 )    !! Hydro
!!write(103, rec=1) StateA % w(1:nlongs, 50)    !! Hydro
!!write(201, rec=1) StateA % w(270, 1:nlevs)     !! Hydro
!!write(202, rec=1) StateA % w(540, 1:nlevs)     !! Hydro
!!write(203, rec=1) StateA % w(810, 1:nlevs)     !! Hydro
...
DO t = 1, ntimesteps
  ...
  IF ((t/dumpspacing)*dumpspacing == t) THEN
    ...
    CALL Write_state_2d (fileout, StateB, dims, ndumps+1, t/dumpspacing, dumpspacing, .NOT.ExtraDiagFields)
  ENDIF
  ...
  !print *, t, ((t/fft_dumpspacing)*fft_dumpspacing == t)
  !IF ((t/fft_dumpspacing)*fft_dumpspacing == t) THEN
    !write(101, rec=t/fft_dumpspacing+1) StateA % w(1:nlongs, 10 )    !! Hydro
    !write(102, rec=t/fft_dumpspacing+1) StateA % w(1:nlongs, 30 )    !! Hydro
    !write(103, rec=t/fft_dumpspacing+1) StateA % w(1:nlongs, 50)    !! Hydro
    !write(201, rec=t+1) StateA % w(270, 1:nlevs)     !! Hydro
    !write(202, rec=t+1) StateA % w(540, 1:nlevs)     !! Hydro
    !write(203, rec=t+1) StateA % w(810, 1:nlevs)     !! Hydro
  !END IF
