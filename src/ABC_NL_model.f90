...
USE DefConsTypes, ONLY :               &
  ...
  Adv_tracer,                          &
  Lv, Moist_On, Ev_option, CO_thresh   ! mosit by Hydro
...
!!!! Moist by Hydro
IF ( Moist_On .eqv. .TRUE. ) THEN   !!  phase change turned on in namelist
  !******************************
  !*****    Advect q        *****
  !******************************
  
  ! This part is same as the Advect Tracer part, but with B * gradient
  ...
  DO k = 1, nlevs
    DO i = 1,nlongs
      ...
      IF (flag_u_at_b) THEN
        upstr_u = u_at_b(i,k) * ( statein % q(i+1,k) - statein % q(i,k) ) * recipdx
      ELSE
        upstr_u = u_at_b(i,k) * ( statein % q(i,k) - statein % q(i-1,k) ) * recipdx
      END IF
      IF (flag_w_at_b) THEN
        upstr_w = aves % w_m(i,k) * ( statein % q(i,k+1) - statein % q(i,k) ) *    &
                  Dims % recip_full_kp1_k(k)
      ELSE
        upstr_w = aves % w_m(i,k) * ( statein % q(i,k) - statein % q(i,k-1) ) *    &
                  Dims % recip_full_k_km1(k)
      END IF
  
      stateout % q (i,k) = statein % q(i,k) - B_dt * ( upstr_u + upstr_w )   ! vec(u) . grad q
    END DO
  END DO
  ...
  DO k = 1, nlevs
    DO i = 1,nlongs
      ...
      IF (flag_u_at_b) THEN
        upstr_u = u_at_b(i,k) * ( statein % qc(i+1,k) - statein % qc(i,k) ) * recipdx
      ELSE
        upstr_u = u_at_b(i,k) * ( statein % qc(i,k) - statein % qc(i-1,k) ) * recipdx
      END IF
      IF (flag_w_at_b) THEN
        upstr_w = aves % w_m(i,k) * ( statein % qc(i,k+1) - statein % qc(i,k) ) *    &
                  Dims % recip_full_kp1_k(k)
      ELSE
        upstr_w = aves % w_m(i,k) * ( statein % qc(i,k) - statein % qc(i,k-1) ) *    &
                  Dims % recip_full_k_km1(k)
      END IF
  
      stateout % qc (i,k) = statein % qc(i,k) - B_dt * ( upstr_u + upstr_w )
    END DO
  END DO
  
  CALL Boundaries (stateout, set_q=.TRUE., set_qc=.TRUE.)
  
  !*******************************
  !*****  phase change       *****
  !*******************************

  CALL Moist(stateout, dims)       !! Calculate Trial delta q
  CALL Boundaries (stateout, set_b=.TRUE., set_q=.TRUE., set_qc=.TRUE.)
