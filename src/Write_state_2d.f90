...
INTEGER, SAVE                          :: varid_q, varid_qc, varid_RH, varid_le       !! Hydro																																			
...
INTEGER                                :: ierr99a, ierr99b, ierr99c, ierr99d    !!  Hydro  
...
IF (t .EQ. 0) THEN
  ...
  ierr5 = NF_DEF_DIM(ncid, 'time', NF_UNLIMITED, dimid_time)  !! Change to unlimited dimension to enable file larger than 2Gb by Hydro
  ...
  IF (Moist_On) THEN
    ...
    ddC(2)  = dimidFull_levs
    ddC(3)  = dimid_time
    ierr99a   = NF_DEF_VAR(ncid, 'q', NF_DOUBLE, 3, ddC, varid_q)
    
    ddC(1)  = dimidLongs_v
    ddC(2)  = dimidFull_levs
    ddC(3)  = dimid_time
    ierr99b   = NF_DEF_VAR(ncid, 'qc', NF_DOUBLE, 3, ddC, varid_qc)
    
    ddC(1)  = dimidLongs_v
    ddC(2)  = dimidFull_levs
    ddC(3)  = dimid_time
    ierr99c   = NF_DEF_VAR(ncid, 'RH', NF_DOUBLE, 3, ddC, varid_RH)
	IF((ierr99a + ierr99b + ierr99c) /= 0) STOP 'ierr99abc'
  END IF
  !! End Moist by Hydro						
  ...
  ELSE
    ...
    IF (Moist_On) THEN
	  ierr99d = NF_DEF_VAR(ncid, 'le',  NF_DOUBLE, 1, ddA, varid_le)  !!by Hydro
	  IF (ierr99d /= 0) STOP 'ierr99d'
    END IF
...
! MOIST Hydro
IF(Moist_On) THEN
  ierr99a   = NF_PUT_VARA_DOUBLE(ncid, varid_q,          startC, countC, state % q(1:nlongs, 1:nlevs))
  ierr99b   = NF_PUT_VARA_DOUBLE(ncid, varid_qc,         startC, countC, state % qc(1:nlongs, 1:nlevs))
  ierr99c   = NF_PUT_VARA_DOUBLE(ncid, varid_RH,         startC, countC, state % RH(1:nlongs, 1:nlevs))
  IF((ierr99a + ierr99b + ierr99c) /= 0) STOP 'ierr99abc'
END IF
! END MOIST Hydro
...
ELSE
  ...
  !Hydro  
  IF(Moist_On) THEN
    ierr99d = NF_PUT_VARA_DOUBLE(ncid, varid_le, startA, countA, state % Latent_energy)
	IF (ierr99d /= 0) STOP 'ierr99d'
  END IF
END IF
