...
! This routine computes the inner product between two states that are in the format
! of that used for the spectral space of the rfft1f/b routines.
! Only the zero and positive wavenumbers are represented.  For the bulk wavenumbers, the
! arrays contain 2x the Fourier transform values, and the negative wavenumber spectrum values are
! inferred as the complex conjugate of the positive wavenumber spectrum values.
...
!=================================================================================================
COMPLEX(ZREAL8) FUNCTION InnerProdControlSpaceNM (x1, x2,                            &
                                                  do_1, do_2, do_3, do_4, do_5)

! Inner product for control space when the control variables are normal mode-based
...
COMPLEX(ZREAL8)                :: total_1, total_2, total_3, total_4, total_5
LOGICAL                        :: flag_1, flag_2, flag_3, flag_4, flag_5
LOGICAL                        :: some_flags

COMPLEX(ZREAL8)                :: InnerProdComplexSpaceNM
...
IF (flag_1) THEN
  total_1 = InnerProdComplexSpaceNM(x1 % v1(0:nlongs+1,1:nlevs), x2 % v1(0:nlongs+1,1:nlevs))
ELSE
  total_1 = COMPLEX(0.0, 0.0)
END IF

IF (flag_2) THEN
  total_2 = InnerProdComplexSpaceNM(x1 % v2(0:nlongs+1,1:nlevs), x2 % v2(0:nlongs+1,1:nlevs))
ELSE
  total_2 = COMPLEX(0.0, 0.0)
END IF

IF (flag_3) THEN
  total_3 = InnerProdComplexSpaceNM(x1 % v3(0:nlongs+1,1:nlevs), x2 % v3(0:nlongs+1,1:nlevs))
ELSE
  total_3 = COMPLEX(0.0, 0.0)
END IF

IF (flag_4) THEN
  total_4 = InnerProdComplexSpaceNM(x1 % v4(0:nlongs+1,1:nlevs), x2 % v4(0:nlongs+1,1:nlevs))
ELSE
  total_4 = COMPLEX(0.0, 0.0)
END IF

IF (flag_5) THEN
  total_5 = InnerProdComplexSpaceNM(x1 % v5(0:nlongs+1,1:nlevs), x2 % v5(0:nlongs+1,1:nlevs))
ELSE
  total_5 = COMPLEX(0.0, 0.0)
END IF

InnerProdControlSpaceNM = total_1 + total_2 + total_3 + total_4 + total_5

END FUNCTION InnerProdControlSpaceNM



!=================================================================================================
COMPLEX(ZREAL8) FUNCTION InnerProdComplexSpaceNM (x1, x2)
...
REAL(ZREAL8), INTENT(IN) :: x1(0:nlongs+1, 1:nlevs)
REAL(ZREAL8), INTENT(IN) :: x2(0:nlongs+1, 1:nlevs)

INTEGER                  :: k, real_index, imag_index
COMPLEX(ZREAL8)          :: total

! Deal with the largest scale (the x elements are purely real or imaginary, and the inner prod contribution
!                              is real)
total = COMPLEX(SUM(x1(1, 1:nlevs) * x2(1, 1:nlevs)), zero)
...
DO k = 1, nlongs/2-1
  ...
  ! Contribution from positive and negative wavenumbers in one (imag parts of above cancel)
  total      = total + COMPLEX(SUM(x1(real_index, 1:nlevs)*x2(real_index, 1:nlevs)) +   &
                               SUM(x1(imag_index, 1:nlevs)*x2(imag_index, 1:nlevs)),    &
                               0.0) / 2.
END DO

! Deal with the smallest scale (the x elements are purely real or imaginary, and the inner prod contribution
!                               is real)
total = total + COMPLEX(SUM(x1(nlongs, 1:nlevs) * x2(nlongs, 1:nlevs)), zero)

InnerProdComplexSpaceNM = total

END FUNCTION InnerProdComplexSpaceNM
...
!=================================================================================================
REAL(ZREAL8) FUNCTION CompareTranspose (Mat1, Mat2, N)
! Check that Mat1 is the transpose of Mat2 by returning the element with the largest difference
! between Mat1 and Mat2^T

USE DefConsTypes, ONLY : &
  ZREAL8

IMPLICIT NONE
REAL(ZREAL8), INTENT(IN) :: Mat1(1:N, 1:N)
REAL(ZREAL8), INTENT(IN) :: Mat2(1:N, 1:N)
INTEGER,      INTENT(IN) :: N
...
DO i = 1, N
  DO j = 1, N
    IF (DABS(Mat1(i,j) - Mat2(j,i)) > largest) largest = DABS(Mat1(i,j) - Mat2(j,i))
  END DO
END DO

CompareTranspose = largest

END FUNCTION CompareTranspose
