...
INTEGER                   :: varid_q, varid_qc, varid_RH, varid_le   !! moist, Hydro
INTEGER                   :: ierr99a, ierr99b, ierr99c, ierr99d	     !! moist, Hydro		   
...
IF (Moist_On) THEN
  ...
  ierr99a = NF_INQ_VARID(ncid, 'q',  varid_q)
  ierr99b = NF_INQ_VARID(ncid, 'qc', varid_qc)
  ierr99c = NF_INQ_VARID(ncid, 'RH', varid_RH)

  IF ((ierr99a + ierr99b + ierr99c) /= 0 ) THEN
    PRINT*,'ierr99a', ierr99a, NF_STRERROR(ierr99a)
    PRINT*,'ierr99b', ierr99b, NF_STRERROR(ierr99b)
    PRINT*,'ierr99c', ierr99a, NF_STRERROR(ierr99c)
    STOP
  END IF
...
ELSE
  ...
  IF(Moist_On)  THEN
    ierr99d = NF_INQ_VARID(ncid, 'le', varid_le)  ! Hydro
	IF(ierr99d /= 0)     STOP 'ierr99d'  !, ierr99d, NF_STRERROR(ierr99d)
  END IF
END IF
...
! Sort out off-boundary values
! Horizontal dimension
dims % longs_u(0)        = dims % longs_u(1) - (dims % longs_u(2) - dims % longs_u(1))
dims % longs_v(0)        = dims % longs_v(1) - (dims % longs_v(2) - dims % longs_v(1))
dims % longs_u(nlongs+1) = dims % longs_u(nlongs) + (dims % longs_u(nlongs) - dims % longs_u(nlongs-1))
dims % longs_v(nlongs+1) = dims % longs_v(nlongs) + (dims % longs_v(nlongs) - dims % longs_v(nlongs-1))
...
DO z = 1, nlevs
  ...
  IF (Moist_On) THEN
    ...
    ierr99a = NF_GET_VARA_DOUBLE (ncid, varid_q, startC, countC, temp)
    state % q(1:nlongs, z) = temp(1:nlongs)								
								
    ierr99b = NF_GET_VARA_DOUBLE (ncid, varid_qc, startC, countC, temp)
    state % qc(1:nlongs, z) = temp(1:nlongs)

    ierr99c = NF_GET_VARA_DOUBLE (ncid, varid_RH, startC, countC, temp)
    state % qc(1:nlongs, z) = temp(1:nlongs)
...
ELSE
  ...
  IF(Moist_On)  THEN   !!Hydro
	ierr99d = NF_GET_VARA_DOUBLE (ncid, varid_le, startA, countA, state % Latent_Energy)
	IF(ierr99d /= 0)     STOP 'ierr99d' !, ierr99d, NF_STRERROR(ierr99d)
  END IF
