...
REAL(ZREAL8), ALLOCATABLE   :: Dummy_x(:), Dummy_z(:), mean_len_x(:), mean_len_z(:)
REAL(ZREAL8), ALLOCATABLE   :: mean_len
REAL(ZREAL8)                :: this_len, nxpoints_r, nzpoints_r, norm, re, im, weight
INTEGER                     :: ierr, k, xx, zz


ALLOCATE (Dummy_x(1:nxpoints))
ALLOCATE (Dummy_z(1:nzpoints))
ALLOCATE (mean_len_x(1:nzpoints))
ALLOCATE (mean_len_z(1:nxpoints))
...
IF (.NOT.fft_init_x) THEN
  fft_worklen_x = 2*nxpoints
  ALLOCATE(fft_wsave_x(1:fft_worklen_x))
  ALLOCATE(fft_work_x(1:nxpoints))
...
IF (.NOT.fft_init_z) THEN
  fft_worklen_z = 2*nzpoints
  ALLOCATE(fft_wsave_z(1:fft_worklen_z))
  ALLOCATE(fft_work_z(1:nzpoints))
...
DEALLOCATE (Dummy_x, Dummy_z, mean_len_x, mean_len_z)
DEALLOCATE (fft_wsave_x, fft_work_x, fft_wsave_z, fft_work_z)
fft_init_x = .FALSE.
fft_init_z = .FALSE.
